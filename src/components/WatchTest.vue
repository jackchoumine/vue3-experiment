<template>
  <div>
    ...
    <br />
    <span>总价：{{ total }}</span>
    <span>姓名：{{ jack.name }}</span>
    <span>年纪：{{ jack.age }}</span>
  </div>
</template>
<script>
...
export default defineComponent({
  ...
  setup(props, { emit, attrs, slots }) {
    ...
    const jack = reactive({ name: 'JACK', age: 24 })
    watch(
      () => {
        return { age: jack.age, name: jack.name }
      },
      value => {
        console.log(value)
      }
    ...
    // setInterval(() => {
    //   jack.name = Math.random().toString(36).substr(2)
    // }, 1500)
    // setInterval(() => {
    //   jack.age = Math.random() * 20
    // }, 1200)
    return { count, prod, total, num, jack }
  },
})
</script>
